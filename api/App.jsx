...
function EntryModal({ show, onClose, onSave, initial }) {
  ...
  const [form, setForm] = useState(initial || { type:"expense",category:"Alimentação",description:"",amount:"",date:today,recurring:false,notes:"" });
  useEffect(() => { setForm(initial || { type:"expense",category:"Alimentação",description:"",amount:"",date:today,recurring:false,notes:"" }); },[initial,show]);
    ...
    <Modal show={show} onClose={onClose}>
      ...
      <div style={{ display:"flex",flexDirection:"column",gap:"0.8rem" }}>
        ...
        <div>
          ...
          <input value={form.notes||""} onChange={e=>set("notes",e.target.value)} placeholder="Ex: parcela 2/12, referência de setembro…" style={S.input}/>
        </div>
...
function CategoryModal({ show, onClose, onSave }) {
  ...
  const addCat = (type) => {
    const val = type==="expense" ? newExp.trim() : newInc.trim();
    if (!val) return;
    if (type==="expense") { const u=[...new Set([...expCats,val])]; setExpCats(u); setNewExp(""); }
    else { const u=[...new Set([...incCats,val])]; setIncCats(u); setNewInc(""); }
  };
  const removeCat = (type, cat) => {
    if (type==="expense") setExpCats(p=>p.filter(c=>c!==cat));
    else setIncCats(p=>p.filter(c=>c!==cat));
  };
  const save = () => {
    localStorage.setItem("expense_categories", JSON.stringify(expCats));
    localStorage.setItem("income_categories", JSON.stringify(incCats));
    EXPENSE_CATEGORIES.splice(0, EXPENSE_CATEGORIES.length, ...expCats);
    INCOME_CATEGORIES.splice(0, INCOME_CATEGORIES.length, ...incCats);
    ALL_CATEGORIES.splice(0, ALL_CATEGORIES.length, ...[...new Set([...expCats,...incCats])]);
    onSave && onSave();
    onClose();
  };
  ...
  return (
    <Modal show={show} onClose={onClose} maxWidth={520}>
      <h2 style={{ margin:"0 0 1.2rem",fontSize:"1rem",color:"#c8a97e",textTransform:"uppercase",letterSpacing:"0.07em" }}>Gerenciar Categorias</h2>
      {[["expense","Despesas",expCats,setExpCats,newExp,setNewExp],["income","Receitas",incCats,setIncCats,newInc,setNewInc]].map(([type,label,cats,setCats,newVal,setNew])=>(
        <div key={type} style={{ marginBottom:"1.2rem" }}>
          <p style={{ ...S.label, marginBottom:"0.5rem" }}>{label}</p>
          <div style={{ display:"flex",flexWrap:"wrap",gap:"0.4rem",marginBottom:"0.6rem" }}>
            {cats.map(c=>(
              <span key={c} style={{ display:"flex",alignItems:"center",gap:"4px",background:"#2a2018",border:"1px solid #3d342a",borderRadius:"6px",padding:"0.2rem 0.5rem",fontSize:"0.78rem" }}>
                {c}
                <button onClick={()=>removeCat(type,c)} style={{ background:"none",border:"none",color:"#8a7a6a",cursor:"pointer",padding:0,lineHeight:1,fontSize:"0.9rem" }}>×</button>
              </span>
            ))}
          </div>
          <div style={{ display:"flex",gap:"0.4rem" }}>
            <input value={newVal} onChange={e=>setNew(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addCat(type)} placeholder={`Nova categoria de ${label.toLowerCase()}…`} style={{...S.input,marginTop:0,flex:1}}/>
            <button onClick={()=>addCat(type)} style={{ background:"#c8a97e",color:"#0f0c0a",border:"none",borderRadius:"8px",padding:"0 1rem",fontWeight:"600",cursor:"pointer" }}>+</button>
          </div>
        </div>
      ))}
      <button onClick={save} style={{ width:"100%",background:"#c8a97e",color:"#0f0c0a",border:"none",borderRadius:"8px",padding:"0.8rem",fontWeight:"600",cursor:"pointer",fontSize:"0.9rem" }}>Salvar Categorias</button>
    </Modal>
  );
}
...
export default function App() {
  ...
  const [customCategories, setCustomCategories] = useState({ expense: EXPENSE_CATEGORIES, income: INCOME_CATEGORIES });
  ...
  const prevEntries = useMemo(() => allEntries.filter(e=>{ const d=parseDate(e.date); return d.getMonth()===prevM && d.getFullYear()===prevY; }), [allEntries,prevM,prevY]);
  ...
  const trendData = useMemo(() => {
    ...
    for (let i=5; i>=0; i--) {
      ...
      const mes = allEntries.filter(e=>{ const ed=parseDate(e.date); return ed.getMonth()===m2 && ed.getFullYear()===y2; });
      result.push({
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      list = list.filter(e=>e.description.toLowerCase().includes(q)||e.category.toLowerCase().includes(q)||(e.notes||"").toLowerCase().includes(q));
    }
    list.sort((a,b) => {
    ...
    return list;
  }, [entries, filterType, filterCategory, search, sortBy]);
